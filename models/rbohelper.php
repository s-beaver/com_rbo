...
class RbOHelper {
  ...
  // =================================================================
  static function getNextInvNumber() {
    ...
    try {
      $db = JFactory::getDBO ();
      $query = $db->getQuery (true);
      $query->select ("MAX(inv_num)");
      $query->from ("rbo_invoices");
      $query->where ("DATE_FORMAT(inv_date,'%Y')=$year");
      $db->setQuery ($query);
      $newNumber = $db->loadResult ();
      $res = new stdClass ();
      $res->new_inv_num = $newNumber;
      $res->new_inv_date = $currentTime->format('d.m.Y', true);
      echo json_encode ($res);
    } catch ( Exception $e ) {
      JLog::add (get_class ($this) . ":" . $e->getMessage (), JLog::ERROR, 'com_rbo');
    }
